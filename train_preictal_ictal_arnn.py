...
def main():
    ...
    signal_file = (
        cache_folder / "preictal_ictal_training_signals.npy"
    )

    label_file = (
        cache_folder / "preictal_ictal_training_labels.npy"
    )
    ...
    write_log(
        "ORIGINAL ARNN WITH RAW PREICTAL-ICTAL DATA",
        log_file
    )
    ...
    if signals.shape != (1782, 1024, 18):
        raise ValueError(
            f"Unexpected signal shape: {signals.shape}"
        )

    if labels.shape != (1782, 1):
        raise ValueError(
    ...
    write_log(
        "Preictal training segments: "
        f"{int((labels == 0).sum())}",
        log_file
    )

    write_log(
        "Ictal training segments: "
        f"{int((labels == 1).sum())}",
        log_file
    )
